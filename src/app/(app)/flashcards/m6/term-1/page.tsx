...
export default async function M6Term1FlashcardPage() {
  ...
  // ถ้า query พลาด (เช่น ยังไม่ได้รัน migration 0009) เริ่มที่ศูนย์แบบเดิม
  const { data: rows } = await supabase
    ...
    .in(
      "deck_id",
      M6_TERM1_UNITS.map((unit) => unit.deckId),
    );
